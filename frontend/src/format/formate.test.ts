...
describe("Texte", () => {
  it("grüßt nach Tageszeit", () => {
    expect(begruessung("Sven", new Date(2026, 7, 27, 8, 0))).toBe(
      "Guten Morgen, Sven.",
    );
    expect(begruessung("Sven", new Date(2026, 7, 27, 14, 0))).toBe(
      "Guten Tag, Sven.",
    );
    expect(begruessung("Sven", new Date(2026, 7, 27, 20, 0))).toBe(
      "Guten Abend, Sven.",
    );
  });
...
describe("centAusText", () => {
  it("liest deutsche Beträge", () => {
    expect(centAusText("1.250,50")).toBe(125050);
    expect(centAusText("0,01")).toBe(1);
    expect(centAusText("18.375,00")).toBe(1837500);
  });

  it("liest auch ohne Tausenderpunkt und mit Punkt als Dezimaltrennzeichen", () => {
    expect(centAusText("1250,5")).toBe(125050);
    expect(centAusText("1250.50")).toBe(125050);
    expect(centAusText("1250")).toBe(125000);
  });
  ...
  it("liest negative Beträge – ein Nachtrag kann eine Minderung sein", () => {
    expect(centAusText("-1.800,00")).toBe(-180000);
  });

  it("rundet auf ganze Cent", () => {
    expect(centAusText("0,145")).toBe(15);
    expect(centAusText("0,144")).toBe(14);
  });

  it("gibt null für Leeres und Unlesbares – nicht null Euro", () => {
    expect(centAusText("")).toBeNull();
    expect(centAusText("   ")).toBeNull();
    expect(centAusText(null)).toBeNull();
    expect(centAusText("abc")).toBeNull();
    expect(centAusText("-")).toBeNull();
    expect(centAusText("1,2,3")).toBeNull();
  });

  it("ist das Gegenstück zu euro()", () => {
    for (const cent of [0, 1, 99, 12345, 1837500, -180000]) {
      expect(centAusText(euro(cent, false))).toBe(cent);
    }
  });
});
